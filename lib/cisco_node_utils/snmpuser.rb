...
module Cisco
  ...
  class SnmpUser
    ...
    def initialize(name, groups, authproto, authpass, privproto,
      ...
      fail ArgumentError if authpass.empty? and [:sha, :md5].include?(authproto) and instantiate
      fail ArgumentError if not authpass.empty? and not [:sha, :md5].include?(authproto)
      fail TypeError unless privproto.is_a?(Symbol)
      fail TypeError unless privpass.is_a?(String)
      fail ArgumentError if privpass.empty? and [:des, :aes128].include?(privproto) and instantiate
      fail ArgumentError if not privpass.empty? and not [:des, :aes128].include?(privproto)
      fail TypeError unless !!localizedkey == localizedkey # bool check
