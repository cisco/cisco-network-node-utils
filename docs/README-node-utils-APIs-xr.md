...
We'll now go through the types of differences that will cause errors/failures, and how to fix them.  When you've fixed a particular error, run minitest again on that single test until it is working as you intend, then re-run the same test against Nexus to make sure it still works.
...
Similar to the get_command differences above, the feature might have an attribute that is functionally equivalent on Nexus and XR, but have small syntactic differences.  An example of this for bgp neighbor is the "connected-check" attribute which produced the following error on XR:
...
And required the following change in bgp\_neighbor.yaml:
...
This was hopefully a good overview for updating an existing Cisco node_utils API to support IOS XR. It is not intended to cover every possibly case, but might be updated in the future with other common cases.